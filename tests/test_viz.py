...
class TestLabelStyleDict:
    ...
    def test_custom_kwargs_and_boundary_color(self):
        ...
        custom_kwargs = {'linewidth': 3, 'facecolor': (0.1, 0.2, 0.3), 'hatch': 'oo', 'alpha': 0.5}
        
        # Test with custom boundary_color and kwargs
        styles = viz.label_style_dict(labels, boundary_color=custom_boundary_color, **custom_kwargs)
        ...
        styles_edge_override = viz.label_style_dict(labels, boundary_color='yellow', edgecolor='magenta')
    ...
    def test_style_consistency_and_color_cycling(self):
        ...
        assert np.allclose(styles1[labels[0]]['facecolor'], styles2[labels[0]]['facecolor'])
        assert np.allclose(styles1[labels[11]]['facecolor'], styles2[labels[11]]['facecolor'])
        assert not np.allclose(styles1[labels[0]]['facecolor'], styles1[labels[1]]['facecolor'])
...
class TestCoreSegmentPlotMethod:
    def test_segment_plot_produces_correct_plot(self):
        ...
        if plot_kwargs.get('text', False):
            ...
            assert actual_patches > 0, "Should have at least some patches from axvspan or annotations"
        else:
...
class TestPlotSegmentBehavior:
    ...
    def test_plot_segment_text_false(self):
        seg = Segment(beta=[0, 1, 2], labels=['L1', 'L2'])
        fig, ax = viz.plot_segment(seg, text=False, ytick="Label") # ytick adds a text object
    ...
    def test_plot_segment_time_ticks_false(self):
        ...
        assert not isinstance(ax.xaxis.get_major_formatter(), librosa.display.TimeFormatter)
...
class TestPlotIntervalsAndLabelsBehavior:
    ...
    def test_internal_plot_custom_style_map(self):
        ...
        viz._plot_intervals_and_labels(intervals, labels, ax, text=True, style_map=style_map)
        ...
        assert np.allclose(anno_s1.get_bbox_patch().get_facecolor(), (1.0, 1.0, 1.0, 1.0)) # Default white bbox
